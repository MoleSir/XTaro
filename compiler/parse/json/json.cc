...
namespace xtaro::parse
{
    ...
    Json Json::loadFromFile(const std::string& filepath)
    {
        ...
        if (!file.is_open())
            debug->errorWithException("Load json file", util::format("File '%s' does not exit.", filepath.c_str()));
    ...
    Json& Json::operator [] (const int index)
    {
        ...
        if (index < 0) 
            debug->errorWithException("Json array", "Array index < 0.");
    ...
    int Json::size() const
    {
        ...
        debug->errorWithException("Json type", util::format("%s has no size.", Json::_jsonTypeString[this->_type]));
        return -1;
    }
    ...
    {
        if (this->_invalid)
            debug->errorWithException("Json", "Use an invalid json.");
    }
